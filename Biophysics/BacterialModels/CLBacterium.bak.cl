...
float4 quat_prod(float4 a, float4 b) {
  ...
  res.w = -a.x*b.x - a.y*b.y - a.z*b.z + a.w*b.w; //.w denotes the scalar component of the quaternion
  return res;
}
...
// Forces vector to have 0-scalar component. Computes q*v*q^(-1) and forces scalar component to be 0
float4 quat_rot(float4 q, float4 v) {
  v.w = 0.f;
...
__kernel void build_matrix(const int max_contacts, const float muA, const float gamma, __global const float4* centers, __global const float4* dirs,
		__global const float* lens, __global const float* rads, __global const int* n_cts, __global const int* frs, __global const int* tos, __global const float* dists,
		__global const float4* pts, __global const float4* norms,
		__global float8* fr_ents, __global float8* to_ents, __global const float* stiff)
{
...
/**
 * Updates centers with simple sum, dirs with more complicated rotation, lens is simple sum
 * After updating the above, sets dcenters, dangs, and dlens to 0
 */
